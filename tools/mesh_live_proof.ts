...
async function main() {
  ...
  if (!RELAY.includes("/p2p/")) throw new Error("membrane did not return a relay multiaddr");
  ...
  console.log(`A (${A.peerId.toString()}) connected to the live relay, reserving…`);
  ...
  console.log(`✓ B sent a signed frame over the relayed (production) connection`);

  await Promise.race([
    got,
    new Promise<void>((_, rej) => setTimeout(() => rej(new Error("timeout: frame never arrived through the live relay")), 25000)),
  ]);
