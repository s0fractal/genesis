import computeHoloWgsl from "../src/lens/shaders/compute_hologram.wgsl" with { type: "text" };
import holoLensWgsl from "../src/lens/shaders/holo_lens.wgsl" with { type: "text" };
...
try {
    device.createBindGroup({
        ...
        entries: [
            { binding: 0, resource: { buffer: device.createBuffer({size: 64, usage: GPUBufferUsage.STORAGE}) } },
            { binding: 1, resource: { buffer: device.createBuffer({size: 64, usage: GPUBufferUsage.UNIFORM}) } },
