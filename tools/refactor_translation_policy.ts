...
async function run() {
    const targets = [
        { pattern: "src/network/translation_policy_*.ts", dest: "src/network/translation_policy" },
        { pattern: "src/bootstrap/translation_policy_*.ts", dest: "src/bootstrap/translation_policy" },
        { pattern: "tests/translation_policy_*.ts", dest: "tests/translation_policy" }
    ];
    ...
    for (const { pattern, dest } of targets) {
        ...
        for (const file of files) {
            ...
            content = content.replace(/from\s+["'](\.\/|\.\.\/)([^"']+)["']/g, (match, prefix, path) => {
                if (prefix === "./") {
                    ...
                    if (dest === "tests/translation_policy" && path.startsWith("tests/")) {
                         // Tests importing other tests
                         return `from "../../${path}"`;
                    }
                    if (path.startsWith("network/translation_policy_") || path.startsWith("bootstrap/translation_policy_")) {
            ...
            content = content.replace(/from\s+["'](\.\.\/\.\.\/src\/network\/)translation_policy_([^"']+)["']/g, 'from "$1translation_policy/translation_policy_$2"');
            content = content.replace(/from\s+["'](\.\.\/network\/)translation_policy_([^"']+)["']/g, 'from "$1translation_policy/translation_policy_$2"');
            content = content.replace(/from\s+["'](\.\.\/bootstrap\/)translation_policy_([^"']+)["']/g, 'from "$1translation_policy/translation_policy_$2"');
