...
const SIGNALING_URL = Deno.env.get("OMEGA_SIGNALING") ||
  "wss://omega-federation.deno.dev";
...
async function streamToWebSocket() {
  ...
  function connect() {
    console.log(
      `[SporeBridge] Connecting to Federation Mesh: ${SIGNALING_URL}`,
    );
    ws = new WebSocket(SIGNALING_URL);

    ws.onopen = () => {
      console.log(
        `[SporeBridge] 🚀 Connected. Ready to bridge hardware frames.`,
      );
      wsOpen = true;
    };
    ...
    while (true) {
      ...
      if (wsOpen && ws.readyState === WebSocket.OPEN) {
        ...
        if (framesBridged % 10 === 0) {
          console.log(
            `[SporeBridge] 📡 Bridged ${framesBridged} frames to mesh... (latest: ${sporeId})`,
          );
        }
      }
