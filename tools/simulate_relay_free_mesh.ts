...
function emit_warrant_at_origin(
  origin_id: string,
  target: number,
  oracle_bit: number,
  tick: number,
  ...
  log_line(
    `[${origin_id}] origin emits WARRANT_VOTE target=0x${
      target.toString(16)
    } oracle_bit=${oracle_bit} TTL=${frameTtl(f)} trail=0x${
      frameTrail(f).toString(16)
    }`,
  );
  return f;
}

function relay_through_chain(
  start_frame: ReturnType<typeof emit_warrant_at_origin>,
  chain: typeof SPORES,
) {
  let f = start_frame;
  for (const sp of chain) {
    ...
    log_line(
      `[${sp.id}] decision=${code_str} TTL→${ttl} trail→0x${
        trail.toString(16)
      }`,
    );
...
function main() {
  log_line(
    "=== OMEGA-64 Era 1130+1150 — Relay-Free Mesh w/ Adaptive TTL ===\n",
  );
  ...
  log_line(
    `[adaptive-ttl] path_length=${ttl_report.path_length}  reliability=${
      ttl_report.reliability.toFixed(4)
    }  margin=${ttl_report.margin}  safety=${ttl_report.safety}  ⇒ TTL=${ttl_report.final_ttl}`,
  );

  // (1) spore-A originates a warrant vote with the computed TTL.
  const ttl = adaptiveTtl(path);
  const origin_frame = emit_warrant_at_origin(
    "spore-A",
    0xCAFE_BABE >>> 0,
    0,
    100,
  ...
  log_line(
    `[relay] AYE (payloadA=${final_frame.payloadA}) target=0x${
      final_frame.proposalOrTarget.toString(16)
    } oracle_bit=${final_frame.oracleBit}`,
  );
  log_line(
    `[relay] trail digest carries 3-hop signature: 0x${
      frameTrail(final_frame).toString(16)
    }`,
  );
  ...
  for (const sp of SPORES.slice(1)) {
    ...
    log_line(
      `[${sp.id}] heartbeat decision=${
        d.code === FWD_FORWARD ? "FORWARD" : `code=${d.code}`
      } TTL→${frameTtl(d.forwardedFrame)}`,
    );
    hb = d.forwardedFrame;
  }
