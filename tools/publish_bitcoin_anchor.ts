...
async function main() {
  ...
  console.log(`\n⚠️  ACTION REQUIRED:`);
  console.log(
    `Please send at least 0.0001 tBTC to this address to fund the OP_RETURN.`,
  );
  console.log(
    `You can use a free faucet like: https://coinfaucet.eu/en/btc-testnet/`,
  );
  ...
  if (amount <= estimatedFee) {
    console.error(
      `Insufficient funds. Need at least ${estimatedFee} sats, got ${amount} sats.`,
    );
    Deno.exit(1);
  }
  ...
  const embed = bitcoin.payments.embed({
    data: [Buffer.from(GENESIS_PAYLOAD, "utf8")],
  });
  psbt.addOutput({
    script: embed.output!,
    value: 0,
  ...
  try {
    ...
    console.log(
      `View on Mempool.space: https://mempool.space/testnet/tx/${txid}`,
    );
  } catch (err) {
