...
async function syncMap() {
  ...
  for (const dir of targetDirs) {
    try {
      for await (
        const entry of walk(dir, { exts: [".rs", ".ts", ".wgsl", ".md"] })
      ) {
        ...
        while ((match = regex.exec(content)) !== null) {
          ...
          const address = rawAddress.startsWith("oct:")
            ? rawAddress
            : `oct:${rawAddress}`;
  ...
  console.log(
    `%c✅ Sync complete. Discovered ${newNodes.length} pragmas. Added ${added} new nodes.`,
    "color: green",
  );
}
