...
async function main() {
  ...
  await R.start();
  const rAddr = R.getMultiaddrs().map((m: { toString(): string }) => m.toString())
  ...
  console.log(`peer  A: ${A.peerId.toString()} (dialed relay, reserving a slot)`);
  ...
  if (!aCircuit) {
    ...
    console.log(`(no advertised circuit addr; falling back to constructed one)`);
  }
  ...
  const claim = `omega-relay-proof:${A.peerId.toString()}:via:${R.peerId.toString()}`;
  const sig = await crypto.subtle.sign("Ed25519", kp.privateKey, new TextEncoder().encode(claim));
  await stream.send(new TextEncoder().encode(JSON.stringify({ claim, sig: b64(sig) })));
  console.log(`✓ B sent a signed frame to A over the relayed connection`);

  await Promise.race([got, new Promise<void>((_, rej) => setTimeout(() => rej(new Error("timeout: A never received the relayed frame")), 15000))]);
  ...
  console.log(pass
    ...
    : `\n✗ FAILED: ${received.ok ? "received but signature invalid" : "frame not relayed"}`);
  Deno.exit(pass ? 0 : 1);
}
