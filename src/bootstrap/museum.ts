...
async function boot() {
  ...
  if (!navigator.gpu) {
    showFatalError(
      "WebGPU is required for the Living Museum. Please use Chrome 113+ or Edge 113+.",
    );
    return;
  }
  ...
  try {
    if ("requestAdapterInfo" in adapter) {
      ...
      adapterLabel = adapterInfo.vendor || adapterInfo.architecture ||
        "WebGPU Adapter";
    }
  } catch (e) {
  ...
  currentSeed = new DataView(ge.buffer, ge.byteOffset, ge.byteLength).getUint32(
    0,
    true,
  );
  ...
  console.log(
    `🎆 [MUSEUM] Big Bang ignited: 100,000 agents. GE Seed: 0x${
  ...
  // HUD init
  const genesisHex = GENESIS_HASH_LEGACY_V1_0.toString(16).toUpperCase()
    .padStart(8, "0");
  ...
  canvas.addEventListener("mousemove", (e) => {
    if (isMouseDown) injectAttractor(e);
  });
  ...
  globalThis.addEventListener("keydown", (e) => {
    ...
    if (key === " ") {
      ...
      document.getElementById("paused-overlay")!.style.display = isPaused
        ? "flex"
        : "none";
      e.preventDefault();
  ...
  document.getElementById("btn-close-info")!.addEventListener(
    "click",
    () => toggleInfo(false),
  );
...
function frameLoop() {
  ...
  if (now - lastStatsUpdate > 100) {
    const fps = Math.round(
      framesSinceLastStat / ((now - lastStatsUpdate) / 1000),
    );
...
function showFatalError(msg: string) {
  document.body.innerHTML =
    `<div style="display:flex; justify-content:center; align-items:center; height:100vh; background:#000; color:#F55; font-family:monospace; padding:2rem; text-align:center;"><h1>[FATAL] ${msg}</h1></div>`;
}
...
function updateStats(fps: number) {
  ...
  // Attractor count
  const attractorArr = new DataView(
    ptrs.attractorBytes.buffer,
    ptrs.attractorBytes.byteOffset,
    80,
  );
...
function injectAttractor(e: MouseEvent) {
  ...
  const setAttractorFn = ENGINE.wasm?.exports
...
function exportWitness() {
  ...
  for (let i = 0; i < Math.min(attractorCount, 4); i++) {
    ...
    attractors.push({
      matrix: attractorArr.getUint32(offset, true).toString(16).padStart(
        8,
        "0",
      ),
      inverse: attractorArr.getUint32(offset + 4, true).toString(16).padStart(
        8,
        "0",
      ),
