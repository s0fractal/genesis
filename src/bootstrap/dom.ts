...
export const DOM = {
  ...
  statusLabel: document.getElementById("status-label") as
    | HTMLSpanElement
    | null,
  ...
  semanticInputGroup: document.getElementById("semantic-input-group") as
    | HTMLDivElement
    | null,
  replayControls: document.getElementById("replay-controls") as
    | HTMLDivElement
    | null,
  replayPlayButton: document.getElementById("replay-play") as
    | HTMLButtonElement
    | null,
  replayTickSlider: document.getElementById("replay-tick") as
    | HTMLInputElement
    | null,
  replayTickValue: document.getElementById("replay-tick-value") as
    | HTMLSpanElement
    | null,
  replayCompareSelect: document.getElementById("replay-compare") as
    | HTMLSelectElement
    | null,

  // Homeostasis Guard
  hMarkerEntropy: document.getElementById("h-marker-entropy") as
    | HTMLDivElement
    | null,
  hMarkerEnergy: document.getElementById("h-marker-energy") as
    | HTMLDivElement
    | null,
  hEntropyVal: document.getElementById("h-entropy-val") as
    | HTMLSpanElement
    | null,
  hEnergyVal: document.getElementById("h-energy-val") as HTMLSpanElement | null,
  hEndocrineVal: document.getElementById("h-endocrine-val") as
    | HTMLSpanElement
    | null,
  ...
  btnSaveGenesis: document.getElementById("btn-save-genesis") as
    | HTMLButtonElement
    | null,
  btnLoadGenesis: document.getElementById("btn-load-genesis") as
    | HTMLButtonElement
    | null,
  btnConnectWallet: document.getElementById("btn-connect-wallet") as
    | HTMLButtonElement
    | null,
  fileLoadGenesis: document.getElementById("file-load-genesis") as
    | HTMLInputElement
    | null,
};
    ...
    DOM.btnConnectWallet.addEventListener("click", async () => {
      ...
      if (typeof ethWindow.ethereum !== "undefined") {
        try {
          ...
          if (DOM.btnConnectWallet) {
            DOM.btnConnectWallet.innerText = `🟢 ${account.substring(0, 6)}...${
        ...
        alert(
          "Please install MetaMask or another Web3 EVM wallet to connect as a Verifier!",
        );
      }
    });
  }
...
export function updateHomeostasisHUD(
  ...
  hoveredAgent?:
    | { hash: bigint; amp: number; lock: number; ent: number }
    | null,
) {
  const now = performance.now();
  ...
  if (DOM.hEndocrineVal) {
    DOM.hEndocrineVal.replaceChildren(
      `K: ${kuramoto.toFixed(2)} | M: ${mutation.toFixed(1)}`,
    );
  }
  if (DOM.hToposVal) {
    if (hoveredAgent) {
      const hex = hoveredAgent.hash.toString(16).padStart(16, "0");
      DOM.hToposVal.replaceChildren(
        `🎯 LATCHED: 0x${hex} [AMP: ${hoveredAgent.amp} | LCK: ${hoveredAgent.lock} | ENT: ${hoveredAgent.ent}]`,
      );
  ...
  if (DOM.hApexVal && apexData) {
    if (apexData.length === 0) {
      ...
      const out = apexData.slice(0, 3).map((a, i) =>
        `#${i + 1} ⚡${a.energy.toString().padStart(5, "0")} v${
          a.velocity.toFixed(2)
        } [${a.hash.toString().substring(0, 8)}]\n${a.ast}`
      ).join("\n\n");
  ...
  if (DOM.hFluxVal && fluxData) {
    if (fluxData.length === 0) {
      ...
      const out = fluxData.slice(0, 3).map((f) =>
        `S[${f.sector.toString().padStart(2, "0")}] Heat: ${
          f.heat.toFixed(1)
        }° | Dilation: ${f.dilation.toFixed(2)}x [${
          f.avgCredit.toFixed(2)
        } cred]`
      ).join("\n");
