...
class SenateViewer {
    ...
    private handlePlasmid(p: PlasmidPayload) {
        if (p.semanticType === "PROPOSAL" && p.proposalHash && p.proposalDescription) {
    ...
    private render() {
        if (this.proposals.size === 0) {
            this.container.innerHTML = `<div class="empty-state">Listening for OMEGA-64 network packets... (Awaiting Senate Plasmids)</div>`;
            return;
        }
            ...
            card.innerHTML = `
                <div class="proposal-header">
                    <div>
                        ...
                        <span class="proposal-meta">Hash: 0x${prop.hash.toString(16).toUpperCase()} • Votes: ${totalVotes}</span>
                    </div>
