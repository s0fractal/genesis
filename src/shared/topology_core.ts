...
export { phase_distance, signed_phase_delta };
...
export function createTopology(config: LatticeConfig) {
  ...
  return {
    ...
    getIndex: (sector: number, rho: number, harmonic: number) => {
      const s = wrapSectors
        ? wrap_index(sector, sectors)
        : clamp_i32(sector, 0, sectors - 1);
...
function fieldIndex(
  ...
  return tau * elementsPerLayer + harmonic * shape.radialBins * shape.sectors +
    rho * shape.sectors + sector;
}
...
export function createPhaseField(
  ...
  const size = shape.tauDepth * shape.harmonics * shape.radialBins *
    shape.sectors;
    ...
    for (let harmonic = 0; harmonic < shape.harmonics; harmonic++) {
      for (let rho = 0; rho < shape.radialBins; rho++) {
        for (let sector = 0; sector < shape.sectors; sector++) {
          ...
          field.omega[idx] = clamp_i32(
            Math.trunc(state.omega),
            PHASE_MIN_OMEGA,
            PHASE_MAX_OMEGA,
          );
          field.amplitude[idx] = clamp_i32(
            Math.trunc(state.amplitude),
            0,
            PHASE_MAX_AMPLITUDE,
          );
          field.lock[idx] = clamp_i32(
            Math.trunc(state.lock),
            0,
            PHASE_MAX_LOCK,
          );
          field.entanglement[idx] = clamp_i32(
            Math.trunc(state.entanglement),
            0,
            PHASE_MAX_ENTANGLEMENT,
          );
          field.preferredTheta[idx] = state.preferredTheta !== undefined
            ? wrapTheta(state.preferredTheta)
            : 0;
          field.memoryStrength[idx] = state.memoryStrength !== undefined
            ? clamp_i32(Math.trunc(state.memoryStrength), 0, 255)
            : 0;
          field.cellStatus[idx] = state.cellStatus !== undefined
            ? state.cellStatus
            : 0;
          field.plasmids[idx] = state.plasmids !== undefined
            ? state.plasmids
            : 0n;
        }
      }
...
export function assertFieldBounds(field: PhaseField): void {
  for (let i = 0; i < field.theta.length; i++) {
    ...
    if (
      field.entanglement[i] < 0 ||
      field.entanglement[i] > PHASE_MAX_ENTANGLEMENT
    ) {
