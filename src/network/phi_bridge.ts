...
export class PhiBridge {
  ...
  public emitIntent(intent: LiquidIntent): void {
    ...
    const phaseShift = intent.phaseVector.length > 0
      ? (Math.floor(intent.phaseVector[0] * 255) & 255)
      : 0;

    console.log(
      `[Φ-Bridge] Routing Intent -> Attractor(0x${
        intentId.toString(16).toUpperCase()
      }), Energy(${energy}), Phase(${phaseShift})`,
    );
