// OMEGA-64: Era 1960 - Translation Policy Replay Digest Digest Quorum
//
// Era 1950 lets peers announce compact `tpdq` replay-interpretation
// digests. This tracker makes those claims observable as deterministic
// quorum state: current consensus digest, agreement strength, and
// dissenting peers.
...
export const TRANSLATION_POLICY_REPLAY_DIGEST_DIGEST_QUORUM_SCHEMA =
    "OMEGA-1960/v1";

export type ReplayDigestDigestQuorumBand =
    | "none"
    | "lone"
    | "double"
    | "triple+"
...
export interface ReplayDigestDigestClaimObservation {
    peer_id: number;
    claim: TranslationPolicyReplayDigestDigestClaim;
    observed_at_ms: number;
}

export interface ReplayDigestDigestQuorumSnapshot {
    schema: string;
    consensus_digest: number | null;
    consensus_count: number;
    total_claims: number;
    band: ReplayDigestDigestQuorumBand;
    dissenter_peer_ids: number[];
    distinct_digests: number[];
    agreement_q16: number;
    consensus_claim: TranslationPolicyReplayDigestDigestClaim | null;
}

export interface ReplayDigestDigestQuorumOptions {
    ttl_ms: number;
    high_threshold: number;
}

export const DEFAULT_REPLAY_DIGEST_DIGEST_QUORUM_OPTS:
    ReplayDigestDigestQuorumOptions = {
        ttl_ms: 5 * 60 * 1000,
        high_threshold: 3,
    };

export class TranslationPolicyReplayDigestDigestQuorumTracker {
    private observations = new Map<number, ReplayDigestDigestClaimObservation>();

    constructor(
        public readonly opts: ReplayDigestDigestQuorumOptions =
            DEFAULT_REPLAY_DIGEST_DIGEST_QUORUM_OPTS,
    ) {
        if (!Number.isFinite(opts.ttl_ms) || opts.ttl_ms <= 0) {
            throw new Error(
                `TranslationPolicyReplayDigestDigestQuorumTracker: ttl_ms must be positive: ${opts.ttl_ms}`,
            );
        }
        if (!Number.isFinite(opts.high_threshold) || opts.high_threshold < 2) {
            throw new Error(
                `TranslationPolicyReplayDigestDigestQuorumTracker: high_threshold must be >= 2: ${opts.high_threshold}`,
            );
        }
    }
    ...
    snapshot(now_ms: number): ReplayDigestDigestQuorumSnapshot {
        this.evict(now_ms);
        if (this.observations.size === 0) {
            return {
                schema: TRANSLATION_POLICY_REPLAY_DIGEST_DIGEST_QUORUM_SCHEMA,
        ...
        return {
            ...
            total_claims: total,
            band: classifyReplayDigestDigestQuorumBand(
                consensusCount,
...
export function classifyReplayDigestDigestQuorumBand(
    count: number,
    high_threshold: number,
): ReplayDigestDigestQuorumBand {
...
export function replayDigestDigestQuorumBandAtLeast(
    actual: ReplayDigestDigestQuorumBand,
    required: ReplayDigestDigestQuorumBand,
): boolean {
    const rank: Record<ReplayDigestDigestQuorumBand, number> = {
        none: 0,
        lone: 1,
        double: 2,
        "triple+": 3,
...
export function replayDigestDigestQuorumGlyph(
    band: ReplayDigestDigestQuorumBand,
): string {
    switch (band) {
        case "high":
...
function normalizeClaim(
    ...
    return {
        ...
        consensus_interval_hash: claim.consensus_interval_hash >>> 0,
        ...
        final_consensus_digest: claim.final_consensus_digest >>> 0,
    };
}
