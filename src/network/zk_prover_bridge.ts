...
export type ZKProofCallback = (receipt: MitosisReceipt, bundle: ZKProofBundle) => void;
...
export class ZKProverBridge {
    ...
    private async processQueue() {
        ...
        try {
            ...
            const cmd = new Deno.Command("cargo", {
                args: ["run", "--release", "--manifest-path", "omega_zk_host/Cargo.toml"],
                stdin: "piped",
                stdout: "piped",
            ...
            if (output.success) {
                ...
                console.error(`[zk_prover_bridge] SP1 STARK generation failed with code ${output.code}`);
            }
        } catch (e) {
    ...
    public async verifyExternalProof(bundle: ZKProofBundle): Promise<boolean> {
        // @ts-ignore
        if (typeof Deno === 'undefined') {
            console.warn(`[zk_prover_bridge] Cannot verify STARK proof via CLI in browser. Assuming soft-proof only.`);
        ...
        try {
            // @ts-ignore
            const cmd = new Deno.Command("cargo", {
                args: ["run", "--release", "--manifest-path", "omega_zk_host/Cargo.toml", "--", "--verify-only"],
                stdin: "piped",
                stdout: "piped",
                stderr: "inherit"
            });
    ...
    public async generateTickRollup(
        ...
        // @ts-ignore
        if (typeof Deno === 'undefined') {
            console.warn(`[zk_prover_bridge] Cannot generate STARK proof in browser context. Skipping Rollup.`);
            return null;
        }
        ...
        try {
            ...
            const attractors = [];
            const attractorView = new DataView(attractorBytes.buffer, attractorBytes.byteOffset, attractorBytes.byteLength);
            ...
            // @ts-ignore
            const cmd = new Deno.Command("cargo", {
                args: ["run", "--release", "--manifest-path", "omega_zk_host/Cargo.toml", "--", "--rollup"],
                stdin: "piped",
                stdout: "piped",
                stderr: "inherit"
            });
            ...
            if (output.success) {
                ...
                try {
                    ...
                    console.error("Failed to parse whole JSON. Attempting line by line fallback...", e);
                ...
                console.error(`[zk_prover_bridge] Rollup STARK generation failed with code ${output.code}`);
            }
        } catch (e) {
