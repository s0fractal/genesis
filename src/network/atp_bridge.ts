...
export interface IATPBridge {
  ...
  subscribeToCosmicEntropy(
    callback: (
      entropy: {
        kuramoto_base: number;
        kuramoto_diffusion_rate: number;
        hash: string;
      },
    ) => void,
  ): void;
...
export class MockATPBridge implements IATPBridge {
  ...
  constructor(seed: bigint = 42n) {
    if (isProduction()) {
      throw new Error(
        "[MockATPBridge] FATAL: Cannot instantiate MockATPBridge in production mode.",
      );
    }
  ...
  async mintATP(
    ...
    console.log(
      `[ATP Bridge] Minted ${reward} ATP for ${wallet}. TX: ${txHash}`,
    );
  ...
  async burnATP(
    ...
    if (current < amount) {
      throw new Error(
        `[ATP Bridge] Insufficient ATP balance for wallet ${wallet}. Needed: ${amount}`,
      );
    }
    ...
    console.log(
      `[ATP Bridge] Burned ${amount} ATP from ${wallet}. TX: ${txHash}`,
    );
  ...
  subscribeToCosmicEntropy(
    ...
    setInterval(() => {
      ...
      const kuramoto_base = KURAMOTO_BASE_MIN +
        this.rng.nextRange(KURAMOTO_BASE_RANGE);
      const kuramoto_diffusion_rate = KURAMOTO_DIFF_MIN +
        this.rng.nextRange(KURAMOTO_DIFF_RANGE);
...
export class EthersATPBridge implements IATPBridge {
  ...
  async burnATP(
    ...
    try {
      ...
      console.log(
        `[Ethers ATP] Burning ${amount} ATP on-chain... TX: ${tx.hash}`,
      );
      await tx.wait();
  ...
  subscribeToCosmicEntropy(
    ...
    this.provider.on("block", async (blockNumber) => {
      try {
        ...
        const kuramoto_diffusion_rate = KURAMOTO_DIFF_MIN +
          (val2 % KURAMOTO_DIFF_RANGE);

        console.log(
          `[Cosmic Entropy] New Block ${blockNumber} | Hash: ${
            block.hash.substring(0, 10)
          }... | Base: ${kuramoto_base}, Diff: ${kuramoto_diffusion_rate}`,
        );
