...
export function stampOrigin(f: SporeFrame, ttl: number): SporeFrame {
  ...
  out.payloadB = 0;
  out.payloadC = ((out.payloadC & ~TTL_MASK) >>> 0) |
    ((ttl & 0xFF) << TTL_SHIFT) >>> 0;
...
export function decideForward(
  ...
  if (
    ft === FRAME_TYPE_HEARTBEAT &&
    (incoming.proposalOrTarget >>> 0) !== (ourGenesis >>> 0)
  ) {
    return { code: FWD_DROP_NOT_FORWARDABLE, forwardedFrame: out };
  }
  ...
  out.payloadC =
    (((out.payloadC & ~TTL_MASK) >>> 0) | (newTtl << TTL_SHIFT)) >>> 0;
  out.crc32 = computeFrameCrc(out);
