...
> **Author:** OMEGA Senate (autopoietic, see Era 1030). **Date:** 2026-04-25.
...
center) and the V2 META-INTENTION ("Open Protocol RFC-style"), the lattice must
publish a stable, language-agnostic specification of its on-the-wire formats and
invariants so that **any independent implementation** — Rust, Go, Python, Zig,
Solidity, hand-rolled C — can join the mesh on equal terms.

This document is the seed. It will grow only through accepted Senate proposals
(Era 1030+).
...
`m XOR inverse == 0xFFFFFFFF`. Implementations MUST reject any plasmid violating
this rule silently (no error propagation).
...
I-5. **Empty center.** No node has elevated rights. Any node may propose, vote,
or relay; acceptance requires 3 unique AYE peers AND `ayes > nays`.
...
boundary will be v1.0 — at that point the JSON wire format freezes for at least
1000 epochs.
...
- Test vectors: `omega_v2/tests/cross_lang_hash.rs`, `tests/senate_test.ts`,
  `tests/routing_mesh_test.ts`
