...
deno task dev
\`\`\`
Observe the HUD, watch the mitosis events, and wait for Era 1060 to unlock. Once unlocked, your local WebLLM instance (Llama 3.2 1B) will boot up and begin casting live votes into the Senate.

### 3. Deploy the Anchor
If you want to symbolically link the protocol's state to the Bitcoin testnet, use the built-in anchor publisher:
\`\`\`bash
deno run -A tools/publish_bitcoin_anchor.ts
\`\`\`
...
3. **The Center Remains Empty**: No single node has elevated administrative rights. The mesh decides.
