...
Deno.test("dedup: high-priority frame types include WARRANT_VOTE", () => {
...
Deno.test("dedup: warrant vote is high-priority", () => {
...
Deno.test("dedup: same intent produces same key (TTL/trail differ)", () => {
...
Deno.test("dedup: different intent produces different keys", () => {
...
Deno.test("plan: low-priority frame stays single-path", () => {
    const candidates: PathCandidate[] = [
...
Deno.test("plan: no disjoint alternative → primary-only", () => {
    const candidates: PathCandidate[] = [
...
Deno.test("plan: forked candidates excluded from primary/secondary", () => {
    const candidates: PathCandidate[] = [
...
Deno.test("plan: empty input → stable shape, not duplicated", () => {
    const plan = planDiversification([], buildWarrantVote(0xCAFE_BABE >>> 0, 0, true, 1));
    assertEquals(plan.duplicated, false);
});

Deno.test("dedup window: first observation returns false", () => {
    const w = new DedupWindow(4);
    assertEquals(w.seen(0xAAAA), false);
    assertEquals(w.size(), 1);
});

Deno.test("dedup window: second observation returns true", () => {
    const w = new DedupWindow(4);
    w.seen(0xAAAA);
    assertEquals(w.seen(0xAAAA), true);
});

Deno.test("dedup window: capacity bound evicts oldest", () => {
...
Deno.test("dedup window: contains() does NOT insert", () => {
    const w = new DedupWindow(4);
    assertEquals(w.contains(0xBBBB), false);
    assertEquals(w.size(), 0);
});

Deno.test("dedup window: clear empties the window", () => {
    const w = new DedupWindow(4);
...
Deno.test("dedup window: invalid capacity throws", () => {
    assertThrows(() => new DedupWindow(0));
    assertThrows(() => new DedupWindow(-1));
});

Deno.test("dedup: end-to-end — duplicate WARRANT_VOTE arrives, dedup catches it", () => {
