// Era 1920: Translation policy replay digest forensic replay tests.
...
} from "../src/network/translation_policy_replay_digest_forensic_event_adapter.ts";
import {
...
function payload(
    overrides: Partial<TranslationPolicyReplayDigestForensicPayload> = {},
): TranslationPolicyReplayDigestForensicPayload {
    return {
...
Deno.test("replay digest forensic replay: custom kind classifies alternate stream", () => {
    ...
    const c = classifyTranslationPolicyReplayDigestForensicSink(sink, { kind: "rdq" });
    assertEquals(c.classified_events, 1);
    assertEquals(c.ignored_events, 1);
