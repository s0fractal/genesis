...
Deno.test("routing: heartbeat forwards when genesis matches", () => {
    const f = stampOrigin(buildHeartbeat(GENESIS_HASH_V1_0, 1), DEFAULT_TTL);
    const d = decideForward(f, 0x1234_5678, GENESIS_HASH_V1_0);
    assertEquals(d.code, FWD_FORWARD);
});

Deno.test("routing: heartbeat dropped on genesis drift", () => {
    const f = stampOrigin(buildHeartbeat(0xDEAD_BEEF >>> 0, 1), DEFAULT_TTL);
    const d = decideForward(f, 0x1234_5678, GENESIS_HASH_V1_0);
    assertEquals(d.code, FWD_DROP_NOT_FORWARDABLE);
});

Deno.test("routing: ttl=0 dropped", () => {
    const f = stampOrigin(buildWarrantVote(0xCAFE_BABE, 0, true, 0), 0);
    const d = decideForward(f, 0xAAAA, GENESIS_HASH_V1_0);
    assertEquals(d.code, FWD_DROP_TTL_ZERO);
});

Deno.test("routing: three-hop chain decrements TTL", () => {
    let f = stampOrigin(buildWarrantVote(0xCAFE_BABE, 0, true, 0), DEFAULT_TTL);
    for (const s of [0x1111_1111, 0x2222_2222, 0x3333_3333]) {
        const d = decideForward(f, s >>> 0, GENESIS_HASH_V1_0);
        assertEquals(d.code, FWD_DELIVER_LOCAL);
        f = d.forwardedFrame;
    }
...
Deno.test("routing: TTL eventually reaches zero", () => {
    let f = stampOrigin(buildWarrantVote(0xCAFE_BABE, 0, true, 0), DEFAULT_TTL);
    let hops = 0;
    while (true) {
        const d = decideForward(f, (0x1000_0000 + hops) >>> 0, GENESIS_HASH_V1_0);
...
Deno.test("routing: forwarded frame carries valid CRC", () => {
    const f = stampOrigin(buildWarrantVote(0xCAFE_BABE, 1, true, 50), DEFAULT_TTL);
    const d = decideForward(f, 0xBBBB_BBBB >>> 0, GENESIS_HASH_V1_0);
...
Deno.test("routing: trail mix is deterministic", () => {
    assertEquals(
