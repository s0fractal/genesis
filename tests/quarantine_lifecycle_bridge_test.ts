...
Deno.test("bridge: starts inactive", () => {
    const { bridge } = makeLoopAndBridge();
    assertEquals(bridge.isActive(), false);
});

Deno.test("bridge: start/stop lifecycle", () => {
    const { bridge } = makeLoopAndBridge();
    bridge.start();
...
Deno.test("bridge: start is idempotent", () => {
    const { bridge } = makeLoopAndBridge();
    bridge.start();
    bridge.start(); // no-op
    assertEquals(bridge.isActive(), true);
...
Deno.test("bridge: stop without start is no-op", () => {
    const { bridge } = makeLoopAndBridge();
    bridge.stop(); // should not throw
    assertEquals(bridge.isActive(), false);
});

Deno.test("bridge: engagement event triggers excludePeer", () => {
    const { loop, source, bridge } = makeLoopAndBridge();
    bridge.start();
    loop.observePeerAnchor(0xFF, 0x999, T0);
    assertEquals(loop.tracker.peerCount(T0), 1);
    source.dispatch("quarantine-engaged", { peer_id: 0xFF });
    assertEquals(loop.tracker.peerCount(T0), 0);
    assertEquals(loop.tracker.excludedPeers(), [0xFF]);
});

Deno.test("bridge: resolution event triggers includePeer", () => {
    const { loop, source, bridge } = makeLoopAndBridge();
    bridge.start();
    source.dispatch("quarantine-engaged", { peer_id: 0xFF });
    assertEquals(loop.tracker.excludedPeers(), [0xFF]);
    source.dispatch("quarantine-resolved", { peer_id: 0xFF });
    assertEquals(loop.tracker.excludedPeers(), []);
});

Deno.test("bridge: malformed payload (missing peer_id) counted, not crashed", () => {
    const { source, bridge } = makeLoopAndBridge();
    bridge.start();
    source.dispatch("quarantine-engaged", {});
    source.dispatch("quarantine-engaged", { peer_id: "not-a-number" });
    source.dispatch("quarantine-engaged", null);
...
Deno.test("bridge: telemetry counts engagements + resolutions", () => {
    const { source, bridge } = makeLoopAndBridge();
    bridge.start();
    source.dispatch("quarantine-engaged", { peer_id: 0xAA });
    source.dispatch("quarantine-engaged", { peer_id: 0xBB });
    source.dispatch("quarantine-resolved", { peer_id: 0xAA });
    const tele = bridge.telemetry();
...
Deno.test("bridge: stop unsubscribes — events after stop are no-ops", () => {
    const { loop, source, bridge } = makeLoopAndBridge();
    bridge.start();
    bridge.stop();
    source.dispatch("quarantine-engaged", { peer_id: 0xFF });
...
Deno.test("bridge: configurable event names", () => {
    ...
    bridge.start();
    source.dispatch("custom-engaged", { peer_id: 0xFF });
    assertEquals(loop.tracker.excludedPeers(), [0xFF]);
...
Deno.test("end-to-end: warrant fires → quarantine engages → tracker excludes → no re-fire", () => {
