...
Deno.test("warrant: proposal hash is deterministic across calls", () => {
...
Deno.test("warrant: distinct reasons produce distinct hashes", () => {
...
Deno.test("warrant: distinct actions produce distinct hashes", () => {
    const h1 = computeProposalHash(0xCAFE_BABE, ACTION_TERMINATE, "r");
    const h2 = computeProposalHash(0xCAFE_BABE, ACTION_MUTATE, "r");
    assert(h1 !== h2);
});

Deno.test("warrant: oracle bit indices match canonical order", () => {
...
Deno.test("warrant: ayeBitsToOracles round-trips", () => {
...
Deno.test("warrant: predicted warrant matches Codeicide computation", () => {
...
Deno.test("warrant: cross-language proposal hash anchor", () => {
