...
Deno.test("drainMitosisLog: single receipt round-trips", async () => {
  ...
  for (let i = 0; i < 32; i++) {
    view.setUint8(slotOff + 148 + i, parseInt(_h1.slice(i * 2, i * 2 + 2), 16));
  }
...
Deno.test("drainMitosisLog: writer overflow drops oldest entries", async () => {
  ...
  for (let slot = 0; slot < MITOSIS_LOG_CAPACITY; slot++) {
    ...
    const lookback =
      ((50 % MITOSIS_LOG_CAPACITY) - slot - 1 + MITOSIS_LOG_CAPACITY) %
        MITOSIS_LOG_CAPACITY + 1;
...
Deno.test("drainMitosisLog: attractor snapshot round-trips", async () => {
  ...
  assert(
    (receipts[0].attractors[0].matrix ^ receipts[0].attractors[0].inverse) >>>
        0 === 0xFFFFFFFF,
  );
});
