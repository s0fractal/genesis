...
Deno.test("convergence: first observation is single-witness", async () => {
...
Deno.test("convergence: second observation is double-witness", async () => {
  const cd = new ConvergenceDetector();
  const wv = buildWarrantVote(0xCAFE_BABE >>> 0, 0, true, 1);
...
Deno.test("convergence: third observation is triple+", async () => {
  const cd = new ConvergenceDetector();
  const wv = buildWarrantVote(0xCAFE_BABE >>> 0, 0, true, 1);
  cd.observe(wv, "spore-A", NOW);
...
Deno.test("convergence: same delivered_by twice doesn't add to list", async () => {
  const cd = new ConvergenceDetector();
  const wv = buildWarrantVote(0xCAFE_BABE >>> 0, 0, true, 1);
  cd.observe(wv, "spore-A", NOW);
...
Deno.test("convergence: different intents tracked separately", async () => {
  const cd = new ConvergenceDetector();
  cd.observe(buildWarrantVote(0xCAFE_BABE >>> 0, 0, true, 1), "spore-A", NOW);
  cd.observe(
    buildWarrantVote(0xDEAD_BEEF >>> 0, 0, true, 1),
    "spore-A",
    NOW + 50,
  );
  cd.observe(
    buildWarrantVote(0xCAFE_BABE >>> 0, 0, true, 2),
    "spore-A",
    NOW + 100,
  );
...
Deno.test("convergence: stats aggregation", async () => {
  const cd = new ConvergenceDetector();
...
Deno.test("convergence: stats on empty detector", async () => {
  const cd = new ConvergenceDetector();
  const s = cd.stats();
...
Deno.test("convergence: proven_carriers returns sorted unique IDs", async () => {
  const cd = new ConvergenceDetector();
...
Deno.test("convergence: stragglers returns single-witness intents past stale threshold", async () => {
  const cd = new ConvergenceDetector();
  cd.observe(buildWarrantVote(0xAA00 >>> 0, 0, true, 1), "spore-A", NOW);
  cd.observe(
    buildWarrantVote(0xBB00 >>> 0, 0, true, 1),
    "spore-A",
    NOW + 5_000,
  );
...
Deno.test("convergence: capacity bound evicts oldest", async () => {
...
Deno.test("convergence: clear empties the detector", async () => {
  const cd = new ConvergenceDetector();
  cd.observe(buildWarrantVote(0xAA >>> 0, 0, true, 1), "x", NOW);
  cd.clear();
...
Deno.test("convergence: invalid capacity throws", async () => {
...
Deno.test("convergence: heartbeat-and-warrant tracked separately", async () => {
  ...
  cd.observe(
    buildWarrantVote(0xCAFE_BABE >>> 0, 0, true, 1),
    "spore-A",
    NOW + 50,
  );
  assertEquals(cd.snapshot().length, 2);
...
Deno.test("convergence: identical intent via different routing metadata still counts as duplicate", async () => {
  const cd = new ConvergenceDetector();
