...
} from "../src/network/translation_policy_warrant_bridge.ts";
import {
...
Deno.test("description: deterministic and senate-sized", () => {
...
Deno.test("bridge: empty events produce no payloads", () => {
    const bridge = new TranslationPolicyWarrantBridge();
    assertEquals(bridge.issue([], T0), { payloads: [], deduped_peer_ids: [] });
});

Deno.test("bridge: single drift event becomes senate-compatible proposal", () => {
    const bridge = new TranslationPolicyWarrantBridge();
    const result = bridge.issue([drift(0xBB)], T0);
    ...
    assertEquals(payload.proposalHash, senateHash(payload.proposalDescription));
});

Deno.test("bridge: multiple drift events produce sorted input-order payloads", () => {
    const bridge = new TranslationPolicyWarrantBridge();
    const result = bridge.issue([drift(0xBB), drift(0xCC)], T0);
    assertEquals(result.payloads.map((p) => p.target_peer_id), [0xBB, 0xCC]);
});

Deno.test("bridge: dedup window blocks same peer/policy pair", () => {
    const bridge = new TranslationPolicyWarrantBridge({ dedup_window_ms: 1000 });
    const event = drift(0xBB);
    assertEquals(bridge.issue([event], T0).payloads.length, 1);
    const repeat = bridge.issue([event], T0 + 500);
    assertEquals(repeat.payloads.length, 0);
    assertEquals(repeat.deduped_peer_ids, [0xBB]);
});

Deno.test("bridge: dedup window elapsed allows reissue", () => {
    const bridge = new TranslationPolicyWarrantBridge({ dedup_window_ms: 1000 });
    const event = drift(0xBB);
    bridge.issue([event], T0);
    assertEquals(bridge.issue([event], T0 + 1500).payloads.length, 1);
});

Deno.test("bridge: different peer policy hash is a distinct issue", () => {
    ...
    assertEquals(bridge.issue([first], T0).payloads.length, 1);
    assertEquals(bridge.issue([second], T0 + 10).payloads.length, 1);
});

Deno.test("bridge: forget clears a specific policy-pair dedup key", () => {
    const bridge = new TranslationPolicyWarrantBridge({ dedup_window_ms: 60_000 });
    const event = drift(0xBB);
    bridge.issue([event], T0);
    bridge.forget(event);
    assertEquals(bridge.issue([event], T0 + 100).payloads.length, 1);
});

Deno.test("bridge: last issued snapshot sorted by dedup key", () => {
    const bridge = new TranslationPolicyWarrantBridge();
    bridge.issue([drift(0xCC), drift(0xAA), drift(0xBB)], T0);
    const keys = bridge.last_issued_snapshot().map((x) => x.key);
    assertEquals(keys, [...keys].sort());
});

Deno.test("defaults: dedup window is positive", () => {
