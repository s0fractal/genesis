...
Deno.test("aggregator: first heartbeat marks UNKNOWN until classifyAfter", async () => {
    ...
    const f = buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 1);
...
Deno.test("aggregator: two heartbeats with advancing tick → HEALTHY", async () => {
    const a = new LivenessAggregator();
    a.ingest("spore-A", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 1), 1000);
    const rec = a.ingest("spore-A", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 2), 1500);
    assertEquals(rec.health, "healthy");
});

Deno.test("aggregator: wrong genesis hash → FORKED immediately", async () => {
    const a = new LivenessAggregator();
    const wrongGenesis = (GENESIS_HASH_LEGACY_V1_0 ^ 1) >>> 0;
...
Deno.test("aggregator: stalled when tick stays the same across heartbeats", async () => {
    ...
    a.ingest("slow", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 5), 1000);
    a.ingest("slow", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 5), 2000);
    const rec = a.ingest("slow", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 5), 3000);
...
Deno.test("aggregator: lost when silent past maxSilenceMs", async () => {
    ...
    a.ingest("ghost", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 1), 1000);
    a.ingest("ghost", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 2), 2000);
...
Deno.test("aggregator: warrant_vote counter increments", async () => {
    const a = new LivenessAggregator();
    a.ingest("voter", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 1), 1000);
...
Deno.test("aggregator: countByHealth aggregates correctly", async () => {
    ...
    // Two healthy.
    a.ingest("h1", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 1), 1000);
    a.ingest("h1", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 2), 1100);
    a.ingest("h2", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 1), 1000);
    a.ingest("h2", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 2), 1100);
    // One forked.
...
Deno.test("aggregator: forget removes a spore", async () => {
    const a = new LivenessAggregator();
    a.ingest("ephemeral", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 1), 1000);
...
Deno.test("aggregator: snapshot returns a fresh array", async () => {
    const a = new LivenessAggregator();
    a.ingest("s", buildHeartbeat(GENESIS_HASH_LEGACY_V1_0, 1), 1000);
    const snap = a.snapshot();
    snap.pop();
