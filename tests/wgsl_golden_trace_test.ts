...
const computeToroidalSrc = await Deno.readTextFile(
  ...
  for (const conf of CONFIGS) {
    Deno.test({
      ...
      async fn() {
        ...
        const latticePtr =
          (exports.v2_lattice_ptr as CallableFunction)() as number;
        const agentsPtr =
          (exports.v2_agents_ptr as CallableFunction)() as number;
        const lutQ10Ptr =
        ...
        if (DEBUG_WGSL_PARITY) {
          let topHex = "";
          for (let k = 0; k < 32; k++) {
            topHex += uniformBytes[k].toString(16).padStart(2, "0") + " ";
          }
          console.log(`[DEBUG] Hex dump of topologyBuf upload: ${topHex}`);
          console.log(
            `[DEBUG] SINE_LUT[0]=${sineLutBytes[0]} SINE_LUT[64]=${
              sineLutBytes[64]
            }`,
          );
        }
        device.queue.writeBuffer(
          ...
          0,
          uniformBytes.buffer,
          uniformBytes.byteOffset + 32,
          48,
        );
        if (DEBUG_WGSL_PARITY) {
          let agentHex = "";
          for (let k = 0; k < 32; k++) {
            agentHex += gpuAgentBytes[k].toString(16).padStart(2, "0") + " ";
          }
        ...
        device.queue.writeBuffer(
          agentsInBuf,
          0,
          gpuAgentBytes.buffer,
          gpuAgentBytes.byteOffset,
          gpuAgentBytes.byteLength,
        );
        device.queue.writeBuffer(
          sineLutBuf,
          0,
          sineLutBytes.buffer,
          sineLutBytes.byteOffset,
          sineLutBytes.byteLength,
        );
        device.queue.writeBuffer(
          attractorBuf,
          0,
        ...
        for (let i = 0; i < conf.ticks; i++) {
          const sigDataBefore = new DataView(
            uniformBytes.buffer,
            uniformBytes.byteOffset + 32,
            48,
          );
          ...
          let hex = "";
          for (let k = 0; k < 16; k++) {
            hex += uniformBytes[32 + k].toString(16).padStart(2, "0") + " ";
          }
          ...
          if (DEBUG_WGSL_PARITY && i === 3) {
            const a0 = new DataView(
              agentBytes.buffer,
              agentBytes.byteOffset,
              32,
            );
            console.log(
              `[TICK4 INPUT] Agent 0: phase=${a0.getUint32(0, true)} energy=${
                a0.getUint32(4, true)
              } mem=[${a0.getUint32(20, true)},${a0.getUint32(24, true)},${
                a0.getUint32(28, true)
              }]`,
            );
          }
          ...
          if (firstMismatch !== -1) {
            ...
            for (let j = 0; j < gpuTickOut.length; j += 32) {
              for (let k = 0; k < 32; k++) {
                if (gpuTickOut[j + k] !== cpuTickOut[j + k]) {
                  mismatchCount++;
                  break;
                }
              }
            ...
            const cpuA = new DataView(
              cpuTickOut.buffer,
              cpuTickOut.byteOffset + agentIdx * 32,
              32,
            );
            const gpuA = new DataView(
              gpuTickOut.buffer,
              gpuTickOut.byteOffset + agentIdx * 32,
              32,
            );
            const sigData = new DataView(
              uniformBytes.buffer,
              uniformBytes.byteOffset + 32,
              48,
            );
            console.log(
              `[DEBUG] abs_tick=${sigData.getUint32(4, true)} active=${
                sigData.getUint32(8, true)
              } mismatching_agents=${mismatchCount}`,
            );
            console.log(
              `[MISMATCH] Tick ${i + 1}: CPU Agent ${agentIdx}: phase=${
                cpuA.getUint32(0, true)
              } energy=${cpuA.getUint32(4, true)} mem=[${
                cpuA.getUint32(20, true)
              }, ${cpuA.getUint32(24, true)}, ${cpuA.getUint32(28, true)}]`,
            );
            console.log(
              `[MISMATCH] Tick ${i + 1}: GPU Agent ${agentIdx}: phase=${
                gpuA.getUint32(0, true)
              } energy=${gpuA.getUint32(4, true)} mem=[${
                gpuA.getUint32(20, true)
              }, ${gpuA.getUint32(24, true)}, ${gpuA.getUint32(28, true)}]`,
            );
            ...
            for (
              let j = 32;
              j < gpuTickOut.length && sampleCount < 4;
              j += 32
            ) {
              let diff = false;
              for (let k = 0; k < 32; k++) {
              ...
              if (diff) {
                const ai = j / 32;
                const cpuS = new DataView(
                  cpuTickOut.buffer,
                  cpuTickOut.byteOffset + j,
                  32,
                );
                const gpuS = new DataView(
                  gpuTickOut.buffer,
                  gpuTickOut.byteOffset + j,
                  32,
                );
                const dp = cpuS.getUint32(0, true) - gpuS.getUint32(0, true);
                console.log(
                  `[SAMPLE] Agent ${ai}: cpu_phase=${
                    cpuS.getUint32(0, true)
                  } gpu_phase=${
                    gpuS.getUint32(0, true)
                  } delta=${dp} cpu_energy=${
                    cpuS.getUint32(4, true)
                  } gpu_energy=${gpuS.getUint32(4, true)}`,
                );
                sampleCount++;
              }
            }
            ...
            console.log(
              `[WGSL DEBUG] attractor_drift=${
                gpuA.getInt32(20, true)
              } weight_left_packed=${gpuA.getUint32(24, true)} weight_right=${
                gpuA.getUint32(28, true)
              }`,
            );
            throw new Error(
              `WGSL drift detected at tick ${
                i + 1
              }, byte ${firstMismatch} (agent ${agentIdx}, field offset ${fieldOffset})`,
            );
          }
