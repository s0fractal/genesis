...
Deno.test({
    ...
    async fn() {
        ...
        try {
            ...
            console.log("⚠️ Puppeteer failed to launch native Chrome. Skipping smoke test.");
            return;
        }
        ...
        try {
            ...
            try {
                await page.goto("http://localhost:4173/museum.html", { waitUntil: "networkidle2" });
            } catch (err) {
                console.log("⚠️ Dev server not running on 4173. Skipping Museum smoke test.");
                return;
            }
            ...
            const fpsText = await page.$eval("#stat-fps-value", el => el.textContent);
            console.log(`[TEST] Current FPS: ${fpsText}`);
            assert(fpsText !== "—" && parseInt(fpsText!) > 0, "FPS should be > 0");

            // Check Mode Parity
            const parityText = await page.$eval("#stat-mode-value", el => el.textContent);
            assertEquals(parityText, "TOROIDAL", "Museum should boot in Toroidal parity mode");

            // Test Pause
            await page.keyboard.press(" ");
            const pausedDisplay = await page.$eval("#paused-overlay", el => getComputedStyle(el).display);
