...
Deno.test("genesis: v1.0 anchors match the cryptographic constants", () => {
...
Deno.test("genesis: v1.0 hash matches the Rust source-of-truth", () => {
    assertEquals(computeGenesisHash(ANCHORS_V1_0), GENESIS_HASH_V1_0);
    assertEquals(GENESIS_HASH_V1_0, 0x549A_6307);
});

Deno.test("genesis: verifyGenesisV1 succeeds in this environment", () => {
    assertEquals(verifyGenesisV1(), true);
});

Deno.test("genesis: any anchor drift produces a different hash", () => {
...
Deno.test("genesis: OP_RETURN inscription format is canonical", () => {
...
Deno.test("genesis: protocol version string is frozen", () => {
