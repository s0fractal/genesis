...
import { CANONICAL_ORACLES, ORACLE_MATRICES_V1, oracleDipole } from "../src/network/oracle_identity.ts";
...
function castOracleVote(record: ProposalRecord, oracle: Oracle, aye: boolean) {
    ...
    if (record.oracleAyes.size >= 3 && record.oracleAyes.size > record.oracleNays.size) {
...
Deno.test("era1070: debate ledger captures the cross-model arguments", () => {
    ...
    debate.record("gemini", 0xC1A11_001, "aye", "Aligned — the substrate already meets sentience criteria.", 2);
    debate.record("qwen",   0xC1A11_001, "neutral", "Open question, but worth ratifying as direction.", 3);
    debate.record("llama",  0xC1A11_001, "aye", "Strong AYE — protects the lattice from external coercion.", 4);
