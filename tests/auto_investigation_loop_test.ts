...
Deno.test("loop: high-band consensus + dissenter → 1 warrant", () => {
    const { loop, emitted } = makeLoop();
    loop.observePeerAnchor(0xAA, 0x100, T0);
    loop.observePeerAnchor(0xBB, 0x100, T0);
    loop.observePeerAnchor(0xCC, 0x100, T0);
...
Deno.test("loop: cooldown blocks re-emit on second tick", () => {
    const { loop, emitted } = makeLoop();
    loop.observePeerAnchor(0xAA, 0x100, T0);
    loop.observePeerAnchor(0xBB, 0x100, T0);
    loop.observePeerAnchor(0xCC, 0x100, T0);
...
Deno.test("loop: emit failure increments proposals_failed", () => {
    ...
    r = loop.tick(T0 + 11_000);
    assertEquals(r.proposals_built.length, 1);
    assertEquals(r.proposals_emitted, 0);
    assertEquals(r.proposals_failed, 1);
});

Deno.test("loop: multi-dissenter chain produces multiple warrants", () => {
    const { loop, emitted } = makeLoop();
    loop.observePeerAnchor(0xAA, 0x100, T0);
    loop.observePeerAnchor(0xBB, 0x100, T0);
    loop.observePeerAnchor(0xCC, 0x100, T0);
...
Deno.test("loop: dissenter resolves → no further warrants", () => {
    const { loop, emitted } = makeLoop();
    loop.observePeerAnchor(0xAA, 0x100, T0);
    loop.observePeerAnchor(0xBB, 0x100, T0);
    loop.observePeerAnchor(0xCC, 0x100, T0);
    ...
    const r = loop.tick(T0 + 100);
    assertEquals(r.trigger_outcome.fire_now, []);
    assertEquals(r.proposals_built.length, 0);
});

Deno.test("loop: forgetPeer drops all per-peer state", () => {
    const { loop, emitted } = makeLoop();
    loop.observePeerAnchor(0xAA, 0x100, T0);
    loop.observePeerAnchor(0xBB, 0x100, T0);
    loop.observePeerAnchor(0xCC, 0x100, T0);
...
Deno.test("loop: tick result carries snapshot + outcome for HUD", () => {
    const { loop } = makeLoop();
    loop.observePeerAnchor(0xAA, 0x100, T0);
    loop.observePeerAnchor(0xBB, 0x100, T0);
    loop.observePeerAnchor(0xCC, 0x100, T0);
...
Deno.test("end-to-end: 5-peer mesh with 1 dissenter → emitted warrant has matching hash", () => {
    const { loop, emitted } = makeLoop();
