...
Deno.test("equivalence hash: ignores observed timestamp", async () => {
    const a = drift(T0);
...
Deno.test("classify confidence bands", async () => {
...
Deno.test("confidenceAtLeast: ordered predicate", async () => {
...
Deno.test("tracker: single witness is lone", async () => {
...
Deno.test("tracker: two distinct witnesses produce double", async () => {
    const t = new TranslationPolicyCorroborationTracker();
...
Deno.test("tracker: same witness duplicate is idempotent", async () => {
    const t = new TranslationPolicyCorroborationTracker();
...
Deno.test("tracker: high-confidence callback fires once", async () => {
    let fired = 0;
    const t = new TranslationPolicyCorroborationTracker({
...
Deno.test("tracker: list orders by witness count then first observed", async () => {
    const t = new TranslationPolicyCorroborationTracker();
    const a = drift(T0);
...
Deno.test("tracker: capacity evicts oldest drift", async () => {
...
Deno.test("tracker: byConfidence filters by minimum band", async () => {
    const t = new TranslationPolicyCorroborationTracker();
    t.record(drift(T0), 0x01, T0);
    t.record(drift(T0 + 1), 0x02, T0 + 1);
...
Deno.test("raise: build + validate", async () => {
...
Deno.test("raise: tampered drift hash rejected", async () => {
    const raise = buildTranslationPolicyCorroborationRaise(drift(), 0xC0, T0);
...
Deno.test("tracker: recordRaise validates and applies witness", async () => {
    const t = new TranslationPolicyCorroborationTracker();
    const raise = buildTranslationPolicyCorroborationRaise(drift(), 0xC0, T0);
...
Deno.test("tracker: recordRaise rejects invalid raise", async () => {
    const t = new TranslationPolicyCorroborationTracker();
    const raise = buildTranslationPolicyCorroborationRaise(drift(), 0xC0, T0);
...
Deno.test("tracker: invalid opts throw", async () => {
