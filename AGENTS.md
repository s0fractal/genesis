...
- **Era 1040 Phase 3 (optional)** — full SP1 prover: replace local re-derivation with STARK proofs generated by `cargo prove`. The kernel infrastructure (Mode 2, MitosisLog, parent snapshotting) is already in place; only the SP1 toolchain hookup remains.
