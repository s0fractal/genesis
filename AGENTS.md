...
### Open Trigger
- **Era 1490: ConvergenceDriver Auto-Pipeline** — top-level orchestration: when `ConvergenceDriver::select_targets` yields a peer, the driver automatically issues HASH_REQUEST, awaits HASH_RESPONSE on a deadline, computes diff via `compute_missing_indices`, and ships only missing entries. State machine on top of Era 1480's primitives so callers stop writing the request/response/diff/ship dance manually.
