...
### Open Trigger
- **Era 1550: Live Mesh Auto-Investigation Loop** — wire `QuorumInvestigationTrigger` + `QuorumWarrantBridge` together inside `WebRTCV2Mesh` so the full autonomous loop runs in production: peers broadcast HASH_LIST → quorum tracker observes → trigger fires → bridge emits proposal plasmid → existing oracle gate adjudicates → quarantine engages. Smoke test: 5-peer mesh with 1 deliberate dissenter, expect 3-of-5 oracle vote → quarantine.
