...
### Open Trigger
- **Era 1620: Cross-Domain Translation Bridge** — for the inevitable case of needing to sync between two different *minor* versions of the same major (e.g. v1.5 emits a new payload field that v1.0 ignores), define an explicit translation contract per schema-pair so operators can specify allowed cross-version transforms instead of just permitting/refusing.
