...
### Open Trigger
- **Era 1580: Multi-Sink Aggregator** — generalize the autonomous-investigation pathway to operate on multiple parallel `ForensicEventSink`s (e.g. one per cluster or per security domain); single `MultiSinkInvestigator` evaluates each sink's quorum independently with shared peer state, surfacing per-sink + aggregated telemetry.
