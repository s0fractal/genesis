...
### Open Trigger
- **Era 1570: Quarantine Lifecycle Bridge** — listen for the existing `era1080-quarantine-engaged` / `era1080-quarantine-resolved` CustomEvents (or equivalents from `auto_investigation.ts`) and automatically call `loop.excludePeer` / `includePeer`; full feedback closure with no operator action.
