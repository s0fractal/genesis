...
### Open Trigger
- **Era 1520: Live Mesh Wiring + Event Plasmid Dispatch** — modify `WebRTCV2Mesh.tsx` to instantiate a `WebRTCEventBridge` on construction, route incoming `EVENT_SYNC` plasmids through `decodeMeshPayload` + `bridge.handleIncoming`, and expose a public `meshBridge` accessor for HUD wiring. End-to-end: 3-peer JSDOM smoke test where event sets converge across the mesh.
