...
### Open Trigger
- **Era 1600: Schema-Validated Cross-Sink Sync** — extend Era 1390's `event_sink_sync.ts` to require schema compatibility between sender and receiver before applying a delta; reject mismatched-name or mismatched-major exchanges with a typed error; integrate with `SinkSchemaRegistry` so the live mesh has a single source of truth for sink schemas.
