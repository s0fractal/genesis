...
### Open Trigger
- **Era 1530: Quorum-Driven Investigation Trigger** — when `EventChainQuorumTracker.dissenters` returns ≥1 peer outside a high-band consensus, automatically raise an investigation (Era 1210 `WARRANT_PROPOSAL`) targeting the dissenters; close the loop "anchor disagreement → forensic investigation → resolution".
