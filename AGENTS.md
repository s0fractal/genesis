...
### Open Trigger
- **Era 1540: Auto-Warrant Issuance from Quorum Trigger** — wire Era 1530's `fire_now` output into Era 1090's `warrant_issuance` flow: each fired peer becomes the target of a fresh `WARRANT_PROPOSAL` plasmid; the existing 3-of-5 oracle warrant gate provides the human-in-the-loop checkpoint. Closes the autonomous loop "anchor disagreement → quorum signal → trigger → warrant proposal → oracle vote → quarantine".
