...
### Open Trigger
- **Era 1610: Schema-Aware Multi-Sink Wiring** — extend `MultiSinkInvestigator` (Era 1580) to wrap each registered sink with `SchemaAwareSinkSync` and validate cross-sink merges through the new schema gate; integrate with `SinkSchemaRegistry` (Era 1590) so the multi-sink layer has automatic schema introspection + telemetry per content domain.
