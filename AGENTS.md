...
### Open Trigger
- **Era 1630: Translation-Aware Sink Sync** — wire `SchemaTranslatorRegistry` into the schema-validated apply path: when `applyEventDeltaWithSchema` would refuse on major-mismatch, consult the registry; if a translator is registered, run it over the delta's records before delegating to Era 1390's apply. Closes the autonomous-translation loop end-to-end.
