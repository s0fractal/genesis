...
### Open Trigger
- **Era 1560: Live Mesh Wiring of AutoInvestigationLoop** — instantiate `AutoInvestigationLoop` inside `WebRTCV2Mesh`, plug `tick` into the existing periodic timer, route emitted warrants through `enqueuePlasmid` as PROPOSAL plasmids, and surface `loop.tick()` result in HUD telemetry. JSDOM smoke test for the production wiring.
