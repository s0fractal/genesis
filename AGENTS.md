...
### Open Trigger
- **Era 1590: Sink Schema Versioning** — define a `ForensicSinkSchema` (e.g. `"financial-events:v1"`, `"system-alarms:v1"`) so multi-sink deployments can mark per-sink content domains explicitly; reject cross-sink delta merges where schemas don't match; track per-schema convergence telemetry.
