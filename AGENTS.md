...
### Open Trigger
- **Era 1510: WebRTCV2Mesh Integration** — wire `WebRTCEventBridge` into the existing mesh's `WebRTCV2Mesh` class so the bridge runs on real `RTCDataChannel` traffic; introduce a `'EVENT_SYNC'` plasmid semanticType that envelopes `BridgeMessage` JSON; smoke-test through a synthetic 3-peer mesh.
